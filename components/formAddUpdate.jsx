...
const FormAddUpdate = ({ project, update = false }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className='p-6 flex flex-col gap-4 md:gap-0 overflow-y-auto min-h-fit'>
      ...
      <button className='bg-tertiary hover:bg-tertiary/80 w-full rounded-md text-center px-4 py-3 flex flex-row items-center justify-center gap-3'><span className='loading loading-spinner' />
        Crear proyecto y Iniciar la simulación
      </button>
    </form>
  )
}
